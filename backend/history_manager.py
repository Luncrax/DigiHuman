"""
Chat History Manager for DigiHuman
========================
This module contains functions for managing chat history,
similar to open-llm-vtuber's chat_history_manager.py
"""
        ...
        for filename in os.listdir(conf_dir):
            ...
            try:
                with open(filepath, "r", encoding="utf-8") as f:
                    ...
                    # Handle empty histories by providing default information
                    if not actual_messages:
                        # Get timestamp from metadata
                        metadata = messages[0] if messages and messages[0]["role"] == "metadata" else {}
...
class HistoryManager:
    """History manager class that wraps the history functions for WebSocket handler"""
    
    def __init__(self):
        # Use the current user ID as the default conf_uid
        import getpass
        self.default_conf_uid = getpass.getuser()
        # Create the chat history directory if it doesn't exist
        if not os.path.exists("chat_history"):
            os.makedirs("chat_history")
    
    def create_new_history(self, client_uid: str) -> str:
        """Create a new history for a client"""
